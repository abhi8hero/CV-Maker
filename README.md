![image](https://github.com/user-attachments/assets/db2249e4-056f-478e-9d27-b8084e5a31cc)

A simple and efficient web application that helps users create professional CVs quickly and easily.


## 📖 About the Project

**CV Maker** is a frontend-based web application designed to simplify the process of building a professional curriculum vitae.  
Users can enter their personal and professional details and generate a clean, well-structured CV layout.

This project was built to practice DOM manipulation, form handling, and real-world UI design using core web technologies.


## 🚀 Live Demo
🔗 Live Demo:  https://abhi8hero.github.io/CV-Maker/                                                                                                                                                               
🚧 Currently running locally / on Github                                                                                                                                              


## ✨ Features
- User-friendly CV creation form
- Real-time CV preview
- Clean and professional CV layout
- Easy customization
- Print-ready design


## 🛠️ Tech Stack
- **HTML5** – Structure
- **CSS3** – Styling
- **JavaScript** – Logic & interactivity

## 📸 Screenshots
<img width="1919" height="908" alt="image" src="https://github.com/user-attachments/assets/6bad89c0-a62d-41b1-8fd6-f8909862b541" />
<img width="1917" height="912" alt="image" src="https://github.com/user-attachments/assets/6a29ccf9-fa3d-43d9-afa7-9a9d9c8b6b8f" />
<img width="1917" height="130" alt="image" src="https://github.com/user-attachments/assets/b51f5257-a22b-49eb-9552-42d486eb44e5" />



## 👨‍💻 Developed By

Abhishek Ugare                                                                                                                                                                         
Email: abhishekugare1289@gmail.com                                                                                                                                                                
LinkedIn: www.linkedin.com/in/abhishek-ugare-1021302a9                             
Portfolio: ####

